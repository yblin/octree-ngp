...
public:
    ...
    NerfNetwork(uint32_t n_pos_dims,
                ...
                std::shared_ptr<TriangleOctree> octree = nullptr)
        ...
        if (octree &&
            pos_encoding.contains("otype") &&
            tcnn::equals_case_insensitive(pos_encoding["otype"], "Takikawa")) {
            m_pos_encoding.reset(new TakikawaEncoding<precision_t, 16>(
                pos_encoding["starting_level"],
                octree,
                tcnn::string_to_interpolation_type(pos_encoding.value("interpolation", "linear")))
            );
        } else {
            m_pos_encoding.reset(tcnn::create_encoding<T>(n_pos_dims,
                                                          pos_encoding,
    ...
    std::unique_ptr<tcnn::Context> forward_impl(cudaStream_t stream,
        ...
        forward->density_network_ctx =
                m_density_network->forward(stream,
                                           forward->density_network_input,
                                           &forward->density_network_output,
                                           use_inference_params,
                                           prepare_input_gradients);
		...
		if (output) {
            tcnn::linear_kernel(extract_density<T>, 0, stream,
                                ...
                                m_dir_encoding->preferred_output_layout() == tcnn::AoS ?
                                    forward->density_network_output.stride() : 1,
                                padded_output_width(),
