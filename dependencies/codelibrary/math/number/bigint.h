...
#ifndef CODELIBRARY_MATH_NUMBER_BIGINT_H_
#define CODELIBRARY_MATH_NUMBER_BIGINT_H_
...
public:
    ...
    BigInt& Assign(const std::string& str, int radix = 10) {
        CHECK(radix >= 2 && radix <= 36);
        CHECK(!str.empty()) << "The input string is empty.";
        ...
        if (str[0] == '+') {
            sign = 1;
            begin = 1;
            CHECK(str.size() > 1) << "The input string is invalid.";
        } else if (str[0] == '-') {
            sign = -1;
            begin = 1;
            CHECK(str.size() > 1) << "The input string is invalid.";
        ...
        // If radix is 16 or 2, special treat.
        if (radix == 2) {
            CHECK(str.size() / RADIX_BITS + 1 <= INT_MAX);
    ...
    /**
     * Convert BigInt to uint32. The sign is ignored.
     */
    ...
    /**
     * Convert BigInt to uint64. The sign is ignored.
     */
...
private:
    ...
    static uint32_t CharToUInt(char character, int radix) {
        CHECK(isdigit(character) || isalpha(character))
        ...
        CHECK(res < radix) << "Invalid input string.";
    ...
    void AddData(const BigInt& a, const BigInt& b) {
        ...
        CHECK(size_ <= MAX_SIZE) << "Overflow detected.";
    }
...
#endif // CODELIBRARY_MATH_NUMBER_BIGINT_H_
