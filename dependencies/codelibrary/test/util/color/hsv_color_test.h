...
#ifndef CODELIBRARY_TEST_UTIL_COLOR_HSV_COLOR_TEST_H_
#define CODELIBRARY_TEST_UTIL_COLOR_HSV_COLOR_TEST_H_

#include <cmath>

#include "codelibrary/base/testing.h"
#include "codelibrary/util/color/hsv_color.h"

namespace cl {
namespace test {

TEST(HSVColorTest, ConvertRGBColorTest) {
    ...
    Array<double> saturation = {
        0.0,   0.0,   0.0,   1.0,   1.0,   1.0,   0.5,   0.5,   0.667, 0.779,
        0.887, 0.828, 0.944, 0.792, 0.661, 0.467, 0.875, 0.75,  0.316
    };
    Array<double> value = {
        1.0,   0.5,   0.0,   1.0,   0.75,  0.5,   1.0,   1.0,   0.75, 0.643,
        0.918, 0.675, 0.941, 0.897, 0.931, 0.998, 0.795, 0.597, 0.721
    };

    for (int i = 0; i < rgb_colors.size(); ++i) {
        HSVColor c(rgb_colors[i]);
        ...
        ASSERT_EQ_NEAR(c.value(), value[i], 1e-3)
                << i << " " << c.value() << " " << value[i];
...
#endif // CODELIBRARY_TEST_UTIL_COLOR_HSV_COLOR_TEST_H_
