...
#ifndef CODELIBRARY_PLOT_SUB_PLOT_H_
#define CODELIBRARY_PLOT_SUB_PLOT_H_
...
public:
    ...
    void SetPlot(int m, int n, const PlotType& plot) {
        static_assert(std::is_base_of<BasePlot, PlotType>::value,
                      "The input parameter must be derived form Plot");
        CHECK(m >= 0 && m < plots_.shape(0));
        CHECK(n >= 0 && n < plots_.shape(1));
...
protected:
    ...
    ArrayND<std::shared_ptr<BasePlot>> plots_;
};

} // namespace plot
} // namespace cl

#endif // CODELIBRARY_PLOT_SUB_PLOT_H_
