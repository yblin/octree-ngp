...
#ifndef CODELIBRARY_PLOT_BASE_PLOT_H_
#define CODELIBRARY_PLOT_BASE_PLOT_H_

#include <string>

#include "codelibrary/string/string_split.h"
#include "codelibrary/plot/axis.h"
#include "codelibrary/plot/font.h"
#include "codelibrary/plot/legend.h"
#include "codelibrary/plot/svg_terminal.h"

namespace cl {
namespace plot {

/**
 * Base plot for drawing geometric objects on XY axes.
 *
...
public:
     ...
     *
     * User can specify the length and width of the plot.
     */
    ...
    void DrawLegend(Terminal* terminal) {
        ...
        if (legend_.position() == Legend::RIGHT_TOP) {
            for (int i = 0; i < n_items; ++i) {
                ...
                item.Draw(cl::RBox2D(x, x + width, y3, y2), terminal);
            ...
            for (int i = n_items - 1; i >= 0; --i) {
                ...
                item.Draw(RBox2D(x, x + width, y3, y2), terminal);
                terminal->DrawText(x - 0.5 * height, y2, item.name);
            }
...
#endif // CODELIBRARY_PLOT_BASE_PLOT_H_
