//
// Copyright 2019-2021 Yangbin Lin. All Rights Reserved.
//
...
#ifndef CODELIBRARY_UTIL_LIST_INDEXED_LIST_H_
#define CODELIBRARY_UTIL_LIST_INDEXED_LIST_H_
...
class IndexedList {
    ...
    public:
        ...
        virtual void Resize(int size) override {
            data_.resize(size, initial_value_);
        }
...
public:
    ...
    IndexedList(const IndexedList& list) = delete;

    IndexedList& operator =(const IndexedList& list) = delete;
    ...
    void Deallocate(Node* node) {
        CHECK(node);
        CHECK(node->id_ < n_allocated_);

        int pos = id_to_index_[node->id_];
        CHECK(pos < n_available_) << "Invalid object: pos = " << pos
                                  << ", n_available = " << n_available_;
     ...
     * Clone indexed list.
     *
     * Note that, we do not copy the properties. But the properties of 'list'
     * will be resized.
     */
    ...
    Property<T> AddProperty(const std::string& name, const T& initial_value) {
        if (property_map_.find(name) != property_map_.end()) {
            CHECK(false) << "The property is already exist.";
        }
    ...
    Property<T> GetProperty(const std::string& name) const {
        ...
        if (iter == property_map_.end()) {
            CHECK(false) << "The property of '" << name << "' is not exist.";
        }

        auto it = property_type_.find(name);
        CHECK(it != property_type_.end());

        CHECK(std::string(typeid(PropertyArray<T>).name()) == it->second)
            << "The value type of property '" << name << "' do not match "
            << typeid(T).name();
    ...
    Node* operator[] (int id) {
        CHECK(id >= 0 && id < n_allocated_);
        CHECK(id_to_index_[id] < n_available_);
...
private:
    ...
    // Object pool to handle the memory.
    Pool<Node> pool_;
...
#endif // CODELIBRARY_UTIL_LIST_INDEXED_LIST_H_
