...
#ifndef CODELIBRARY_GEOMETRY_BEZIER_CURVE_2D_H_
#define CODELIBRARY_GEOMETRY_BEZIER_CURVE_2D_H_
...
public:
     ...
     */
    BezierCurve2D(const Array<Point>& control_points)
        : control_points_(control_points) {
        CHECK(control_points_.size() >= 2);
    }

    /**
     * Split a Bezier curve into two curves for given ratio t.
     *
     ...
     * numerically stable.
     */
    void Split(const T& t, BezierCurve2D* curve1, BezierCurve2D* curve2) const {
        CHECK(t >= 0 && t <= 1);
        CHECK(curve1 && curve2);
...
using FBezierCurve2D = BezierCurve2D<float>;
using RBezierCurve2D = BezierCurve2D<double>;

} // namespace cl

#endif // CODELIBRARY_GEOMETRY_BEZIER_CURVE_2D_H_
