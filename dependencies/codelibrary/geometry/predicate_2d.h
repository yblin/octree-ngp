...
#ifndef CODELIBRARY_GEOMETRY_PREDICATE_2D_H_
#define CODELIBRARY_GEOMETRY_PREDICATE_2D_H_
 ...
 */
template <typename T>
T OrientationDeterminant(const T& px, const T& py, const T& qx, const T& qy,
...
int Orientation(const Point2D<T>& p, const Point2D<T>& q, const Point2D<T>& r) {
    ...
    // Exact computation.
    ExactFloat det2 = OrientationDeterminant(ExactFloat(p.x), ExactFloat(p.y),
 ...
 */
template <typename T>
T IncircleDeterminant(const T& px, const T& py, const T& qx, const T& qy,
    ...
    T t1 = Determinant(pqx, pqy, ptx, pty);
    ptx *= qtx;
    pty *= qty;
    ptx += pty;

    T t2 = Determinant(pqx, pqy, prx, pry);
    prx *= qrx;
    pry *= qry;
    prx += pry;

    return Determinant(t1, ptx, t2, prx);
}
...
int InCircle(const Point2D<T>& p, const Point2D<T>& q,
    ...
    // Exact computation.
    ExactFloat det = IncircleDeterminant(ExactFloat(p.x), ExactFloat(p.y),
                                         ExactFloat(q.x), ExactFloat(q.y),
                                         ExactFloat(r.x), ExactFloat(r.y),
...
#endif // CODELIBRARY_GEOMETRY_PREDICATE_2D_H_
