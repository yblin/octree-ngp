...
#ifndef CODELIBRARY_GEOMETRY_PREDICATE_3D_H_
#define CODELIBRARY_GEOMETRY_PREDICATE_3D_H_
 ...
 */
template <typename T>
T OrientationDeterminant(const T& px, const T& py, const T& pz,
    ...
    return Determinant(pqx, pqy, pqz, prx, pry, prz, psx, psy, psz);
}

/**
 * Compute the orientation from s lies above the plane of pqr.
 *
...
int Orientation(const Point3D<T>& p, const Point3D<T>& q,
    ...
    // Protect against overflow in the computation of det.
        ...
        double det = Determinant(pqx, pqy, pqz, prx, pry, prz, psx, psy, psz);
        if (det > eps)  return +1;
        if (det < -eps) return -1;
    }
...
/**
 * Compute the determinant for InSphere test, i.e.,
 *
 ...
 */
template <typename T>
T InSphereDeterminant(const T& px, const T& py, const T& pz,
                      const T& qx, const T& qy, const T& qz,
                      const T& rx, const T& ry, const T& rz,
                      const T& sx, const T& sy, const T& sz,
    ...
    return Determinant(ptx, pty, ptz, pt2, rtx, rty, rtz, rt2,
...
int InShpere(const Point3D<T>& p, const Point3D<T>& q,
    ...
    // Protect against overflow in the computation of det.
        ...
        double det = Determinant(ptx, pty, ptz, pt2, rtx, rty, rtz, rt2,
                                 qtx, qty, qtz, qt2, stx, sty, stz, st2);
        if (det > eps)  return +1;
        if (det < -eps) return -1;
    }
    ...
    // Exact computation.
    ExactFloat det =
            InSphereDeterminant(ExactFloat(p.x), ExactFloat(p.y),
                                ExactFloat(p.z),
                                ExactFloat(q.x), ExactFloat(q.y),
                                ExactFloat(q.z),
                                ExactFloat(r.x), ExactFloat(r.y),
...
#endif // CODELIBRARY_GEOMETRY_PREDICATE_3D_H_
