...
#include "codelibrary/world/instance_node.h"
...
#include "codelibrary/world/tree/leaf_generator.h"
...
class ProceduralTree : public Node {
    ...
    struct Stem {
        ...
        FVector3D orientation;

        // Unit face orientation of this stem.
        FVector3D face_orientation;
        ...
        void Roll(float degree) {
            FQuaternion rot(orientation, DegreeToRadian(degree));
        ...
        void GlobalRoll(float degree) {
            FQuaternion rot(FVector3D(0.0f, 0.0f, 1.0f),
        ...
        void Pitch(float degree) {
            FVector3D v = CrossProduct(orientation, face_orientation);
            FQuaternion rot(v, DegreeToRadian(degree));
            orientation = rot * orientation;
            face_orientation = rot * face_orientation;
        }
    };
...
public:
    /**
     * Parameter for procedural tree model.
     */
    struct Parameter {
        // Integer 0-8, controls shape of the tree by altering the first level
        // branch length. Predened options conical, spherical, hemispherical,
        // cylindrical, tapered cylindrical, flame, inverse conical, tend flame
        // and custom respectively. Custom uses the envelope defined by the
        // prune_* parameters to control the tree shape directly rather than
        // through pruning.
        int shape;
        ...
        // How drastically the branch radius is reduced between levels.
        float ratio_power;

        // How much the radius at the base of the trunk increases as a fraction
        // of the trunk radius.
        float flare;

        // Number of splits at base height on trunk, if negative then the number
        // of splits will be randomly chosen up to a maximum of |base_splits|.
        int base_splits = 0;

        // Control the angle of the direction of a child branch (at level n)
        // away from that of its parent (at level n - 1).
        Array<float> down_angle;

        // Maximum variation in down_angle[n], if < 0 then the value of down
        // angle v[n] is distributed along the parent.
        Array<float> down_angle_v;

        // Proportion of branch on which no child branches/leaves are spawned.
        Array<float> base_size;

        // Angle about the parent branch (at level n - 1) between each child
        // branch.
        // If < 0 then child branches are directed rotate[n] degrees away from
        // the downward direction in their parent local basis.
        // For fanned branches, the fan will spread a total angle of rotate[n]
        // and for whorled branches, each whorl will rotate by rotate[n].
        Array<float> rotation;

        // Maximum variation in rotate_angle[n]. For fanned and whorled
        // branches, each branch will vary in angle by rotate v[n].
        Array<float> rotation_v;

        // The maximum number of child branches at level n on each parent stem.
        Array<int> branches;

        // The length of branches at level n as a fraction of their parent
        // branch's length.
        Array<float> length;

        // Maximum variation in length[n].
        Array<float> length_v;

        // Controls the tapering of the radius of each branch along its length.
        //
        // 0   -  Non-tapering cylinder
        // 0~1 -  Branch tapers to that fraction of its base radius at its end.
        // 1   -  Taper to a cone.
        // 1~2 -  Interpolate between conical tapering and this rounded end.
        // 2   -  Taper to a spherical end.
        // 2~3 -  Result in periodic tapering with a maximum variation in radius
        //        equal to the value − 2 of the base radius.
        // 3   -  A series of adjacent spheres.
        Array<float> taper;

        // Maximum number of dichotomous branches at each segment of a branch,
        // fractional values are distributed along the stem using a method
        // similar to Floyd-Steinberg error diffusion.
        Array<float> segment_splits;

        // Angle between dichotomous branches.
        Array<float> split_angle;

        // Maximum variation in split angle[n].
        Array<float> split_angle_v;

        // Number of segments in each branch (stem).
        Array<int> curve_resolution;

        // Angle by which the direction of the stem will change from start to
        // end, rotating about the stem's local x-axis.
        Array<float> curve;

        // Maximum variation in curve[n]. Applied randomly at each segment.
        Array<float> curve_v;

        // Angle in the opposite direction to curve[n] that the stem will curve
        // back from half way along, creating S shaped branches.
        Array<float> curve_back;

        // Maximum angle by which the direction of the stem may change from
        // start to end, rotating about the stem's local y-axis.
        // Applied randomly at each segment.
        Array<float> bend_v;

        // Control the distribution of branches along their parent stem.
        // 0 indicates fully alternate branching, interpolating to fully
        // opposite branching at 1.
        // Values > 1 indicate whorled branching with n + 1 branches in each
        // whorl. Fractional values result in a rounded integer number of
        // branches in each whorl, with the difference propagated using a method
        // similar to Floyd-Steinberg error diffusion.
        Array<float> branch_dist;

        // Modify the base radius of branches, only for use in special cases
        // such as the weeping willow where the standard radius model is not
        // sufficient.
        Array<float> radius_modify;

        // Number of leaves or blossom on each of the deepest level of branches.
        int n_leaves = 0;

        // Predefined leaf shape.
        int leaf_shape = 0;

        // Scale of leaves.
        float leaf_scale = 1.0f;

        // X direction scale of leaves.
        float leaf_scale_x = 1.0f;

        // Fractional amount by which leaves are reoriented to face the light
        // (upwards and outwards).
        float leaf_bend = 0.0f;

        // Influence upon the growth direction of the tree in the x, y and z
        // directions, the z element only applies to branches in the second
        // level and above (n >= 2).
        FVector3D tropism = FVector3D(0.0f, 0.0f, 0.0f);

        // The fractional distance from the bottom of the pruning up at which
        // the peak width occurs.
        float prune_width_peak = 0.5f;

        // The curvature of the lower section of the pruning envelope.
        // < 1 results in a convex shape, > 1 in concave.
        float prune_power_low = 0.5f;

        // The curvature of the upper section of the pruning envelope.
        // < 1 results in a convex shape, > 1 in concave.
     ...
     */
    void Generate(const Parameter& parameter) {
...
private:
    ...
    void AddStems(const Stem& stem) {
        if (stem.depth >= stems_lod_) return;
    ...
    void CreateTree() {
        ...
        tree_.clear();
        leaves_.clear();
        leaves_node_.ClearInstances();

        if (parameter_.branches[0] == 0) return;
        ...
        Array<FPoint3D> roots;
        if (parameter_.branches[0] == 1) {
            roots.emplace_back(0.0f, 0.0f, 0.0f);
        } else {
            ...
            for (int i = 0; i < parameter_.branches[0]; ++i) {
                ...
                while (!point_ok) {
                    ...
                    FPoint3D pos(dis * std::cos(theta), dis * std::sin(theta),
                                 0.0f);

                    // Test point against those already in array to ensure it
                    // will not intersect.
                    bool flag = true;
                    for (const FPoint3D& p : roots) {
        ...
        for (const FPoint3D& root : roots) {
            ...
            stem.orientation   = FVector3D(0.0f, 0.0f, 1.0f);
            stem.root          = root;
            if (roots.size() > 1) {
                ...
                stem.face_orientation = FVector3D(std::cos(angle),
                                                  std::sin(angle),
                                                  0.0f);
            }
     ...
     * Update stem object.
     */
    void UpdateStemObject() {
        stem_object_.clear();
        for (const Stem& stem : tree_)
            AddStems(stem);
    }
    ...
    void GenerateLeaves() {
        ...
        for (const LeafBlossom& l : leaves_) {
            ...
            transform.Translate(l.position.ToVector());
            FQuaternion q(FVector3D(0.0f, 0.0f, 1.0f), l.orientation);
            FVector3D v = q * FVector3D(0.0f, 1.0f, 0.0f);
            FQuaternion q1(v, l.face_orientation);
            gl::Transform transform1(q1 * q);

            if (bend > 0.0f) {
                float theta_pos = std::atan2(l.position.y, l.position.x);
                float theta_bend = theta_pos - std::atan2(l.face_orientation.y,
                                                          l.face_orientation.x);
                FQuaternion q2(FVector3D(0.0f, 0.0f, 1.0f), theta_bend * bend);
                gl::Transform transform2(q2);
    ...
    void SetupLeafNode() {
        ...
        for (FPoint3D& p : leaf_render_data.vertices) {
            p *= scale;
            p.x *= scale_x;
        }
    ...
    void MakeStem(int start,
                  ...
                  Stem* stem) {
        if (start >= parameter_.curve_resolution[stem->depth]) return;

        // If the stem is so thin as to be invisible then don't bother to make
        // it.
        if (stem->radius_limit < 0.0001f) return;
        ...
        // Start point.
        FPoint3D start_point = stem->root;
    ...
    float CalculateRadius(const Stem& stem, float z_norm) const {
        ...
        if (n_taper < 1.0f) {
            ...
            if (n_taper < 2.0f) {
                ...
                z3 = std::abs(z2 - 2.0f * taper *
                              std::round(z2 / (2.0f * taper)));
            }
            if (n_taper < 2.0f && z3 >= taper) {
                radius = taper;
            } else {
                radius = (1.0f - depth) * taper +
                         depth * std::sqrt(std::pow(taper, 2.0f) -
                                           std::pow((z3 - taper), 2.0f));
            }
        }
        ...
        if (stem.depth == 0) {
            ...
            float flare = parameter_.flare *
                          ((std::pow(100.0f, y_val) - 1.0f) / 100.0f) + 1.0f;
            radius *= flare;
        }
        return radius;
    }
    ...
    void ApplyTropism(Stem* stem) {
        FVector3D v = parameter_.tropism;
        if (stem->depth <= 1) v.z = 0.0f;

        if (v.x != 0.0f || v.y != 0.0f || v.z != 0.0f) {
            FVector3D v2 = CrossProduct(stem->orientation, v);
    ...
    float CalculateBranchCount(const Stem& stem) const {
        ...
        if (stem.depth == 0) {
            ...
            result = parameter_.branches[depth_1] *
                    (0.2f + 0.8f * (stem.length / stem.parent_length) /
                     stem.max_length);
        } else {
            result = parameter_.branches[depth_1] *
    ...
    float GetRotateAngle(int n, float prev_angle) const {
        ...
        if (parameter_.rotation[n] >= 0.0f) {
            r_angle = prev_angle + GetRandom(parameter_.rotation[n],
                                             parameter_.rotation_v[n]);
        } else {
            r_angle = prev_angle * (180.0f +
                                    GetRandom(parameter_.rotation[n],
                                              parameter_.rotation_v[n]));
        }
        r_angle = std::fmod(r_angle, 360.0f);
        if (r_angle < 0.0f) r_angle += 360.0f;
        return r_angle;
    }
    ...
    float GetDownAngle(const Stem& stem, float stem_offset) const {
        ...
        if (parameter_.down_angle_v[depth_1] >= 0.0f) {
            ...
            float ratio = ShapeRatio(0, (stem.length - stem_offset) /
                                        (stem.length * (1.0f - base_size)));
    ...
    float GetSpreadAngle(float declination) const {
        return -(20.0f + 0.75f * (30.0f + std::abs(declination - 90.0f)) *
                                  std::pow(GetRandomNumber2(), 2.0f));
    }
    ...
    float GetEffectSpreadAngle(int depth, int i, int n_splits) const {
        ...
        return (i + 1) * 360.0f / (n_splits + 1) + GetRandomNumber1() *
               parameter_.split_angle_v[depth];
    }
    ...
    Array<LeafBlossom> leaves_;

    // Render object for stems.
    RenderObject stem_object_;

    // Parameter for generating tree.
    ...
    // Color of leaves.
    RGB32Color leaf_color_ = RGB32Color(90, 170, 20);
